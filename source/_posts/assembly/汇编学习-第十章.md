...
程序分析：
<!-- TODO:确定123, 12366, 1, 8 , 3, 38这些数在内存中是以二进制形式存储的（二进制补码），2个字节存储一个数字。 -->
...
```
3. 在2中把余数按照倒叙放在了栈中，我们需要把他读取出来，变为ASII放到db数据段中（可以使用ds）而且字符的最后是零结束。要注意对边界0的处理。注意：ds:si 指向字符串的首地址。 
